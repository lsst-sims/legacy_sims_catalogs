...
def guessDtype(dataPath, numGuess, delimiter, **kwargs):
    ...
    with open(dataPath) as fh:
        while cnt < numGuess:
            teststr += fh.readline()
            cnt += 1
    dataArr = np.genfromtxt(StringIO(teststr.decode(encoding='ascii')), dtype=None, names=True, delimiter=delimiter, **kwargs)
    ...
    with open(datapath) as fh:
        ...
        for l in fh:
            ...
            if cnt%chunkSize == 0:
                ...
                dataArr = np.genfromtxt(StringIO(tmpstr.decode(encoding='ascii')), dtype=dtype, delimiter=delimiter, **kwargs)
