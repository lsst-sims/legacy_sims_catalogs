...
class InstanceCatalogTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fudge_factor = 2.0*np.degrees(dl)
        # fudge_factor is the increase in radius due to the fact
    ...
    def testStarLike(self):
        ...
        for line in testData:
            ...
            self.assertLess(np.degrees(dl), self.obsMd.boundLength+2.0*self.fudge_factor)
        ...
        for ic in lines_not_in_catalog:
            ...
            self.assertGreater(np.degrees(dl), self.obsMd.boundLength+2.0*self.fudge_factor)
...
class boundingBoxTest(unittest.TestCase):
    ...
    def testCircBounds(self):
        ...
        for line in myIterator:
            ...
            self.assertLess(rtest, self.radius+self.fudge_factor)
        ...
        for line in testData:
            ...
            self.assertLess(dl, self.radius+self.fudge_factor)
        ...
        for line in self.starControlData:
            if line['id'] not in testData['id']:
                ...
                self.assertGreater(dl, self.radius+self.fudge_factor)
